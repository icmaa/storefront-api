...
const redis = require('@storefront-api/lib/redis');
const redisClient = redis.getClient(config)
const countryMapper = require('@storefront-api/lib/countrymapper')
 ...
 */
function processSingleOrder (orderData, config, job, done?: (error?, result?) => void, logger = Logger) {
  const TOTAL_STEPS = 4;
  ...
  const processCart = (result) => {
    ...
    api.cart.pull(null, cartId, null, isThisAuthOrder).then((serverItems) => {
      ...
      Promise.all(syncPromises).then((results) => {
        ...
        let mappedShippingRegion: Record<string, any> = {}
        let mappedBillingRegion: Record<string, any> = {}
        ...
        api.directory.countries().then((countryList) => {
          ...
          const shippingAddressInfo: Record<string, any|Record<string, any>> = { // sum up totals
            addressInformation: {
              billingAddress: {
                countryId: billingAddr.country_id,
                street: billingAddr.street,
