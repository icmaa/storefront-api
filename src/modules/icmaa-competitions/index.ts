...
import { auth as GAuth, sheets as GoogleSheets } from '@googleapis/sheets'
...
module.exports = ({ config }: ExtensionAPIFunctionParameter): Router => {
  ...
  api.post('/form', async (req, res) => {
    ...
    const auth = new GAuth.GoogleAuth({ scopes, clientOptions, credentials })
    ...
    const sheetsApi = GoogleSheets('v4')
